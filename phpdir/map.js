...
function map_init(){
  ...
  for(h = 0; h < n; h++)
  {
    ...
    var txin = htmlspecialchars(tx_arr[0]);
    dt_ar[h] = htmlspecialchars(dt_ar[h]);
    ujb_ar[h] = htmlspecialchars(ujb_ar[h]);
    img_ar[h] = htmlspecialchars(img_ar[h]);
    unm_ar[h] = htmlspecialchars(unm_ar[h]);
 
    var br = "<br/>";
    for(d = 1; d < tx_arr.length; d++)
    txin += "<br/>" + tx_arr[d];
    br += "<br/><br/><br/>";
    $("#map_area").append('<div id=' + bn + ' style="top: '+ cy +'px; left: ' + cx + 'px; width: 200px; height: 150px; min-width: 200px; min-height: 150px; max-width:400px; background-color: white; position: absolute; border: 1px solid #000; overflow: hidden; z-index: ' + h + ' " onMouseOver="document.getElementById(\'' + dvt + '\').style.display=\'\'; this.style.height=\'auto\'; this.style.width=\'auto\'; this.style.zIndex=\'' + n  + '\'; evdis(); " onMouseOut="document.getElementById(\'' + dvt + '\').style.display=\'none\'; this.style.height=\'150px\'; this.style.width=\'200px\'; this.style.zIndex=\'' + h + '\'; evenb(); " ><img src=' + img_ar[h] + ' align="left"></img><span style="font-size:x-small;" >' + dt_ar[h] + '</span><br/><i>' + ujb_ar[h] + '</i><br/><a href="">' + unm_ar[h] + '<a><br clear="left"><div id=' + bnt + ' ></div>' + br + '<div id="' + dvt + '" name="' + dvt + '" style="display:none" ><input id=' + at1 + ' name=' + at1 + ' type="button" onclick="eval(1, ' + h + '); this.disabled=true;" value="創" /><input id=' + at2 + ' name=' + at2 + ' type="button" onclick="eval(2, ' + h + '); this.disabled=true;" value="知" /><input id=' + at3 + ' name=' + at3 + ' type="button" onclick="eval(3, ' + h + '); this.disabled=true;" value="感" /></div></div>');
...
//エリア内フラグ
  ...
  //ボックス外であれば新しいボックスを作る
    ...
    $("#box").append('<textarea id="txtarea" cols="5" rows="3" maxlength="140" placeholder="ここに書きこんでください" ></textarea><div><select name="arrow" id="arrow" ><option value="1">矢印を引く（賛成）<option value="2">矢印を引く（反対）<option value="3">矢印を引く<option value="0">矢印を引かない</select></div><input type="button" id="submit" value="送信" onclick="sub()" /><input type="button" id="cancel" value="取り消し" onclick="can()" />');
...
function sub(){
  ...
  ctm = htmlspecialchars(ctm);
  uimg = htmlspecialchars(uimg);
  ujob = htmlspecialchars(ujob);
  ...
  for(c = 1; c < txarr.length; c++)
    txtn += "*+*" + txarr[c];
  document.getElementById(bont).innerHTML = htmlspecialchars(txt);
  document.getElementById("sub_txt").value = htmlspecialchars(txtn);
  document.getElementById("sub_x").value = w_x;
  document.getElementById("sub_y").value = w_y;
  document.getElementById("sub_cdt").value = htmlspecialchars(ctm);
